...
import org.liveSense.jcr.explorer.client.fileupload.Upload;
import org.liveSense.jcr.explorer.client.fileupload.UploadListener;
...
public class AddNewNode {
	...
	public Window addNewNodeBox(JcrExplorer jackrabbitExplorer) {
	    ...
	    class AddNodeSubmitValuesHandler implements SubmitValuesHandler {  
			...
			public void onSubmitValues(com.smartgwt.client.widgets.form.events.SubmitValuesEvent event) {
	        	if (addNewNodeForm.validate()) {
	        		...
	        		JcrExplorer.service.addNewNode(path, newNodeName.getValue().toString(), newNodeType.getValue().toString(), null,
	    ...
	    class AddNodeButtonHandler implements ClickHandler {  
	    	private final JcrExplorer jackrabbitExplorer;
	    	public AddNodeButtonHandler(JcrExplorer jackrabbitExplorer) {
	    		this.jackrabbitExplorer = jackrabbitExplorer;
	    	}
	        @Override
	    ...
	    class CancelAddNodeButtonHandler implements ClickHandler {  
			...
			public void onClick(ClickEvent event) {  
				...
				JcrExplorer.service.addNewNode("", "", "", null, "", true, new AsyncCallback<String>() {
					@Override
		...
		upload.setAction(JcrExplorer.UPLOAD_SERVLET_PATH);
