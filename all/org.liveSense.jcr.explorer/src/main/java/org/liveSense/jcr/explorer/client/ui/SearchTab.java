...
import org.liveSense.jcr.explorer.client.callback.SearchServiceCallback;
...
//		searchFullTxt.setValidators(regExpValidator);
	    ...
	    class SearchFullTextSubmitValuesHandler implements SubmitValuesHandler {  
	    	...
	    	public void onSubmitValues(com.smartgwt.client.widgets.form.events.SubmitValuesEvent event) {
					if (searchFullTextForm.validate()) {
						...
						JcrExplorer.service.fullTextSearch(searchFullTxt.getValue().toString(),
	...
	private Tab xpathSearchTab(JcrExplorer jackrabbitExplorer) {
	    ...
	    class SearchXpathSubmitValuesHandler implements SubmitValuesHandler {  
	    	private JcrExplorer jackrabbitExplorer;
	    	public SearchXpathSubmitValuesHandler(JcrExplorer jackrabbitExplorer) {
	    		this.jackrabbitExplorer = jackrabbitExplorer;
	    	}
	    	public void onSubmitValues(com.smartgwt.client.widgets.form.events.SubmitValuesEvent event) {
	    		if (searchXpathForm.validate()) {
	    			JcrExplorer.showLoadingImg();
	    			JcrExplorer.service.xpathSearch(searchXpathTxt.getValue().toString(),new SearchServiceCallback(jackrabbitExplorer));
	    		}
	      }  
	    };
	...
	private Tab sqlSearchTab(JcrExplorer jackrabbitExplorer) {
	    ...
	    class SearchSqlSubmitValuesHandler implements SubmitValuesHandler {  
	    	private JcrExplorer jackrabbitExplorer;
	    	public SearchSqlSubmitValuesHandler(JcrExplorer jackrabbitExplorer) {
	    		this.jackrabbitExplorer = jackrabbitExplorer;
	    	}
	    	public void onSubmitValues(com.smartgwt.client.widgets.form.events.SubmitValuesEvent event) {
	    		if (searchSqlForm.validate()) {
	    			JcrExplorer.showLoadingImg();
	    			JcrExplorer.service.sqlSearch(searchSqlTxt.getValue().toString(), new SearchServiceCallback(jackrabbitExplorer));
	    		}
	      }  
	    };
