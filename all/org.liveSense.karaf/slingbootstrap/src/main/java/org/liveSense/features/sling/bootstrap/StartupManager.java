...
public class StartupManager {
	...
	StartupManager(final Map<String, String> properties,
		...
		if ( overrideMode != null ) {
			...
			this.logger.info("Override property set. Starting in mode " + this.mode);
		} else {
			this.mode = detectMode(properties.get(Constants.FRAMEWORK_STORAGE));
			this.logger.info("Detected startup mode. Starting in mode " + this.mode);
		}
		...
		if (dataFile.exists()) {
			...
			try {
				...
				if (selfStamp > 0) {
					...
					logger.info(String.format("Stored startup timestamp: %s", storedStamp));
		...
		if (loader instanceof URLClassLoader) {
			...
			if (urls.length > 0) {
				...
				try {
					...
					if ( stamp > selfStamp ) {
						logger.info(String.format("Newer timestamp for %s from %s : %s", clazz.getName(), url, selfStamp));
						timeStamp = selfStamp;
					}
	...
	private long getSelfTimestamp() {
				...
				if ( jarFiles != null ) {
					for (final File bundleJar : jarFiles) {
						if (bundleJar.lastModified() > selfStamp) {
							...
							logger.info(String.format("Newer timestamp from %s : %s", bundleJar, selfStamp));
						}
					}
				}
			}
		...
		logger.info(String.format("Final self timestamp: %s.", selfStamp));
